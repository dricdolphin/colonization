	...
	function lista_dados($turno_atual=true) {
		...
		foreach ($this->id AS $chave => $valor) {
			...
			if ($ultimo_planeta != $this->id_planeta[$chave]) {
				...
				<div data-atributo='nome_planeta' id='planeta_{$planeta->id}'>
			...
			if ($instalacao[$this->id_instalacao[$chave]]->desguarnecida == 0) {
				$exibe_acoes = "<input data-atributo='pop' data-ajax='true' data-valor-original='{$this->pop[$chave]}' type='range' min='0' max='10' value='{$this->pop[$chave]}' oninput='return altera_acao(event, this);' onchange='return valida_acao(event, this, true);' onmouseup='return valida_acao(event, this);' ontouchend='return valida_acao(event, this);' {$this->disabled}></input>&nbsp;&nbsp;&nbsp;<label data-atributo='pop' style='width: 30px;'>{$this->pop[$chave]}</label>";
			...
			if ($instalacao[$this->id_instalacao[$chave]]->html_especial != "") {
				...
				$this->debug .= "acoes->lista_dados => verifica_html_especial: {$instalacao[$this->id_instalacao[$chave]]->html_especial}\n";
	...
	function pega_balanco_recursos($id_planeta_instalacoes=0, $salva_balanco=false, $imperio = "") {
		...
		foreach ($this->id AS $chave => $valor) {
			...
			foreach ($instalacao[$this->id_instalacao[$chave]]->recursos_produz as $chave_recursos => $id_recurso) {
				...
				if ($imperio->bonus_recurso('*') !== false && $id_recurso !== $id_poluicao) {//Tem bônus para TODOS os recursos
					//Verifica as condições
					if ($imperio->extrativo('*') && !$imperio->bonus_recurso('*')) {//O bônus se aplica somente para recursos EXTRATIVOS
						if ($imperio->extrativo('*') && $recurso[$id_recurso]->extrativo == 1) {
							if ($instalacao[$this->id_instalacao[$chave]]->desguarnecida == 1 && $this->desativado[$chave] == 0) {
				...
				if ($imperio->bonus_recurso($id_recurso) !== false) {//Tem bônus para o recurso ATUAL
					//Verifica as condições
					if ($imperio->extrativo($id_recurso)) {//O bônus se aplica somente para recursos EXTRATIVOS
						if ($imperio->extrativo($id_recurso) && $recurso[$id_recurso]->extrativo == 1) {
							if ($instalacao[$this->id_instalacao[$chave]]->desguarnecida == 1 && $this->desativado[$chave] == 0) {
		...
		foreach ($ids_colonia as $id) {
			if ($flag_novo_balanco) { //Somente calcula o consumo de Alimentos, Energia e Poluição das colônias se for um NOVO balanço
				...
				if ($planeta[$id->id_planeta]->inospito == 0 || $planeta[$id->id_planeta]->terraforma == 1) {
					if (empty($this->recursos_consumidos_planeta[$id_poluicao][$id->id_planeta])) {
						$this->recursos_consumidos_planeta[$id_poluicao][$id->id_planeta] =  $imperio->consome_poluicao;
					} else {
						$this->recursos_consumidos_planeta[$id_poluicao][$id->id_planeta] = $this->recursos_consumidos_planeta[$id_poluicao][$id->id_planeta] + $imperio->consome_poluicao;
					}
				}
			}
